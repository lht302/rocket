...
class RoCCCoreIO(val nRoCCCSRs: Int = 0)(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val dmemaddr = Input(UInt(40.W))
  val yaofull_counter_out = Output(Bool())
  val rowham_addr_up_out     = Output(UInt(40.W))
  val rowham_addr_down_out      = Output(UInt(40.W))
  val rowham_addr_cache_out      = Output(UInt(40.W))
  val rowham_flashen_out  = Output(Bool())
//===== zzguardrrlht: End   ====//
}
...
trait HasLazyRoCCModule extends CanHavePTWModule
  ...
  val (respArb, cmdRouter) = if(outer.roccs.nonEmpty) {
    ...
    outer.roccs.zipWithIndex.foreach { case (rocc, i) =>
      ...
      rocc.module.io.dmemaddr := cmdRouter.io.dmemaddr
      cmdRouter.io.yaofull_counter_in := rocc.module.io.yaofull_counter_out
      cmdRouter.io.rowham_addr_up_in  := rocc.module.io.rowham_addr_up_out
      cmdRouter.io.rowham_addr_down_in  := rocc.module.io.rowham_addr_down_out
      cmdRouter.io.rowham_addr_cache_in  := rocc.module.io.rowham_addr_cache_out
      cmdRouter.io.rowham_flashen_in  := rocc.module.io.rowham_flashen_out
      //===== zzguardrrlht: End   ====//
    }
...
class LooktableExample(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new LooktableExampleModuleImp(this)
}
class LooktableExampleModuleImp(outer: LooktableExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
    with HasCoreParameters {
  ...
  val addr    = WireDefault(2.U(2.W))
  val data_in = WireDefault(0.U(2.W))
  val ren     = WireDefault(false.B)
  val wen     = WireDefault(false.B)

  addr    := io.cmd.bits.inst.funct(6,5)
  data_in := io.cmd.bits.inst.funct(4,3)
  ren     := io.cmd.bits.inst.funct(2).asBool
  wen    := io.cmd.bits.inst.funct(1).asBool

  io.resp.valid := io.cmd.valid
  
  when(io.resp.ready){
    io.resp.bits.data := table.read(addr,ren)
  }
   
  when(wen && io.cmd.valid){
    table.write(addr,data_in)
  }
}
...
class RoccCommandRouter(opcodes: Seq[OpcodeSet])(implicit p: Parameters)
  ...
  val io = IO(new Bundle {
    ...
    val rowham_addr_down_out      = Output(UInt(40.W))
    val rowham_addr_cache_out      = Output(UInt(40.W))
    val rowham_addr_up_in     = Input(UInt(40.W))
    val rowham_addr_down_in    = Input(UInt(40.W))
    val rowham_addr_cache_in    = Input(UInt(40.W))
    val rowham_flashen_out = Output(Bool())
    val rowham_flashen_in = Input(Bool())
//===== zzguardrrlht: End   ====//
  })
 // eliminate queue delay
  //val cmd = Queue(io.in)
  val cmd = io.in
...
io.rowham_addr_up_out  := io.rowham_addr_up_in
io.rowham_addr_down_out  := io.rowham_addr_down_in
io.rowham_addr_cache_out  := io.rowham_addr_cache_in
io.rowham_flashen_out   := io.rowham_flashen_in
//===== zzguardrrlht: End   ====//
