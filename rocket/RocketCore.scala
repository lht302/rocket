...
trait HasRocketCoreIO extends HasRocketCoreParameters {
  ...
  val io = IO(new CoreBundle()(p) {
    ...
    val rowham_addr_up    = Input(UInt(40.W))
    val rowham_addr_down   = Input(UInt(40.W))
    val rowham_addr_cache  = Input(UInt(40.W))
    val rowham_flashen  = Input(Bool())
...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  io.rocc.dmemaddr := io.dmem.req.bits.addr
...
//===lht===rowhammer
  val rowham_corestalld = WireDefault(false.B)
 //===lht==== 
  val ctrl_stalld =
    ...
    rowham_corestalld //rowhammer 处理
  ...
  //=========lht rowhammer dmemaccess :start ======//
...
rowham_req_valid := rowham_req_valid_cache || rowham_req_valid_down_next || rowham_req_valid_up || rowham_req_valid_up2 || rowham_req_valid_up3
//val resp_valid = Wire(Bool())
//val rowham_flashen = Wire(Bool())
//resp_valid := io.dmem.resp.valid
//rowham_flashen := io.rowham_flashen
rowham_corestalld := (state === s_stall || state === s_cache || state === s_up || state === s_up2 || state === s_up3|| state === s_down)
rowham_data := io.dmem.resp.bits.data
switch(state){
    is(s_idle)
     {when(io.rowham_flashen){
      state := s_stall
     }
    }
    is(s_stall)
     {
      when(!(ex_reg_valid || mem_reg_valid || wb_reg_valid)){
    ...
    { rowham_req_valid_cache := false.B
      rowham_dmemaddr := io.rowham_addr_cache 
    when(io.dmem.resp.valid || io.dmem.perf.grant){
    ...
    {rowham_req_valid_up := false.B
     rowham_dmemaddr := io.rowham_addr_up 
    when(io.dmem.resp.valid || io.dmem.perf.grant){
  ...
  is(s_up2)
    {rowham_req_valid_up2 := false.B
     rowham_dmemaddr := io.rowham_addr_up + 2048.U
    when(io.dmem.resp.valid || io.dmem.perf.grant){
  ...
  is(s_up3)
    {rowham_req_valid_up3 := false.B
     rowham_dmemaddr := io.rowham_addr_up + 4096.U
    when(io.dmem.resp.valid || io.dmem.perf.grant){
    ...
    { rowham_req_valid_down := false.B
      rowham_dmemaddr := io.rowham_addr_down 
    when(io.dmem.resp.valid || io.dmem.perf.grant){
    state := s_idle
  ...
  io.dmem.req.valid     := Mux(rowham_corestalld,rowham_req_valid,ex_reg_valid && ex_ctrl.mem)
  val ex_dcache_tag = Cat(ex_waddr, ex_ctrl.fp)
  require(coreParams.dcacheReqTagBits >= ex_dcache_tag.getWidth)
  io.dmem.req.bits.tag  := Mux(rowham_corestalld,0.U,ex_dcache_tag)
  io.dmem.req.bits.cmd  := Mux(rowham_corestalld,0.U,ex_ctrl.mem_cmd)
  io.dmem.req.bits.size := Mux(rowham_corestalld,0.U,ex_reg_mem_size)
  io.dmem.req.bits.signed := Mux(rowham_corestalld,1.U,!Mux(ex_reg_hls, ex_reg_inst(20), ex_reg_inst(14)))
  io.dmem.req.bits.phys := false.B
  io.dmem.req.bits.addr := Mux(rowham_corestalld,rowham_dmemaddr,encodeVirtualAddress(ex_rs(0), alu.io.adder_out))
  ...
  io.dmem.s1_data.data := Mux(rowham_corestalld,0.U,(if (fLen == 0) mem_reg_rs2 else Mux(mem_ctrl.fp, Fill((xLen max fLen) / fLen, io.fpu.store_data), mem_reg_rs2)))
  io.dmem.s1_data.mask := DontCare

  io.dmem.s1_kill := Mux(rowham_corestalld,0.U,killm_common || mem_ldst_xcpt || fpu_kill_mem)
  io.dmem.s2_kill := false.B
  // don't let D$ go to sleep if we're probably going to use it soon
  io.dmem.keep_clock_enabled := ibuf.io.inst(0).valid && id_ctrl.mem && !csr.io.csr_stall


//=========lht rowhammer dmemaccess :end ======//
